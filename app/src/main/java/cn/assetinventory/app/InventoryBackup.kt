...
data class BackupTask(val id:String,val companyId:String,val name:String,val companyName:String,val status:String)
data class BackupPreview(val file:File,val createdAt:Long,val companies:Int,val tasks:Int,val assets:Int,val scans:Int,val photos:Int,val companyDetails:List<BackupCompany>,val taskDetails:List<BackupTask>,val reason:String,val targetType:String,val targetName:String)
object InventoryBackup{
    private const val VERSION=1
    fun create(context:Context,db:InventoryDatabase,reason:String="手动创建完整备份",targetType:String="ALL",targetName:String="全部数据"):File{
        db.writableDatabase.rawQuery("PRAGMA wal_checkpoint(FULL)",null).use{while(it.moveToNext()) {}}
        val database=context.getDatabasePath("asset_inventory.db");val photos=File(context.filesDir,"anomaly_photos").walkTopDown().filter{it.isFile}.toList();val meta=JSONObject().apply{put("format","ASSET_INVENTORY_BACKUP");put("version",VERSION);put("createdAt",System.currentTimeMillis());put("companies",db.companies().size);put("tasks",db.tasks().size);put("assets",db.ledgerAssetCountAll());put("scans",db.tasks().sumOf{db.transferScans(it.id).size});put("photos",photos.size);put("reason",reason);put("targetType",targetType);put("targetName",targetName)}.toString().toByteArray();val dir=File(context.filesDir,"task_packages").apply{mkdirs()};val out=File(dir,"asset_inventory_${System.currentTimeMillis()}.invbackup")
